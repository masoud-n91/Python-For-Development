...
class Admins(SQLModel, table=True):
    ...
    password: str
    is_active: bool
    join_time: datetime = datetime.now(pytz=paris_tz)
...
class Comments(SQLModel, table=True):
    ...
    user_id: int = Field(foreign_key="Guests.id")
    comment: str
    created_at: datetime
    guests: Optional[Guests] = Relationship(back_populates="comments")  # Relationship back to User
...
def submit_comment(user_id, comment):
    ...
    query = f"INSERT INTO comments (user_id, comment, created_at) VALUES (%s, %s, %s)"
